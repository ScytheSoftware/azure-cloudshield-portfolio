...
## 📌 Overview
This module demonstrates a **Zero-Trust data architecture** in Azure SQL Database. It focuses on enforcing the **Principle of Least Privilege (PoLP)** and protecting sensitive Personally Identifiable Information (PII) at rest using **Dynamic Data Masking (DDM)** without altering the underlying raw data.
...
* **Dynamic Data Masking (DDM):** Obfuscates sensitive customer attributes (such as email addresses, credit card numbers, or social security numbers) in query results for unprivileged users.
